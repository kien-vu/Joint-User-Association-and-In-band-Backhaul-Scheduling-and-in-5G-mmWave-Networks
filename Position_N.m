function [ MUEnode,  MUEnode_Ad, HeNBnode, HUEnode] = Position_N( M, S)
...
% Here small cell users are located surround the SCs
  for s = 1:S
        ...
        HUEnode(2,s) =  HeNBnode(2,s) + 1 * (-5+10*rand); 
             while(1)
                if norm(HUEnode(:,s) - HeNBnode(:,s)) > Bound1 ||  norm(HUEnode(:,s) - HeNBnode(:,s)) < 2
                    HUEnode(1,s) =  HeNBnode(1,s) + 1 * (-5+10*rand);
                    HUEnode(2,s) =  HeNBnode(2,s) + 1 * (-5+10*rand); 
                else 
                    break;
...
% Here for MUEs
   for m = 1:M
            ...
            % Need to consider some SCs with many MUEs surrounding
            for s = 1:S
                while(1)
                ...
                if s == 1 && m >= 3/4*M
                    while(1)
                        if norm(MUEnode(:,m) - HeNBnode(:,s)) >= Bound3 || norm(MUEnode(:,m) - HeNBnode(:,s)) <= 2
                                    MUEnode(1,m) =   HeNBnode(1,s) + 1 * (-5+10*rand);
                                    MUEnode(2,m) =   HeNBnode(2,s) + 1 * (-5+10*rand); 
                        else 
                            break;
                        end
                    end
