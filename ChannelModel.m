function [ THETA_tilde_Wired, THETA_tilde_Only_B, THETA_tilde_Wireless, V, HHH, SINR_S2u, IF_Fun, SINR_S2M] = ...
...
% scalefactor = min(scalefactor, noise_p);
temp = 0;
for k=1:K
    if US_Var(k) ~=0
%         temp = min(min(min(abs(HH(k,:)))));
        temp = 10^(3)* max(max(max(abs(HH(k,:)))));
    end
...
% HH = HH/scalefactor;
% V = Uprecoder * Tprecoder ; % Here we design the heriarchical precoding
% in order to mitigate both co- and cross-tier interference. That is very
% important step in this work.
if N_Sc > 0
...
for s = 1:S
     for u = 1:S
        ...
        pathloss_S2u(u,s)= 10^(-pathloss_S2u_dB(u,s)/10);
        ChanG_u(u,s)  =   pathloss_S2u(u,s) * abs(sqrt(N_s) * ( 0*randn + 1i*1/N_s*randn)).^2; 
     end
end
SF3 = (max(max(abs(pathloss_S2u))));
% SF3 = min(min(min(abs(pathloss_S2u))));
...
% SFS2M = min((min(min(abs(pathloss_S2M)))));
...
for m = 1:M
    for s = 1:S
        SINR_S2M(m,s) = 10^(Tx_Gain/10) * P_s * N_s / N_au * norm( ChanG_S2M(m,s) )/min(SFS2M,SF3)/((1 + Epsilon)); % + 20 * log10(abs(ChanG)) + 20 * log10(abs(ChanG(s))) In dB, then calculate the rate easily
    end
end
